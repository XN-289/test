...
### Phase 5 — Verify (LLM-Native)

**Step A — Run Verify.** 
For high-level qualitative goals, `Verify` should be an LLM-driven script that:
1. Aggregates project state (Wiki, Strategy, Site).
2. Invokes an LLM-Grader with a specific Expert Rubric.
3. Extracts a numeric score from the LLM's structured reasoning.

**Standard for "Expertise" Evaluation**:
- **0-30**: Superficial, marketing-heavy, lacks data.
- **31-60**: Contains data, but lacks structural logic.
- **61-80**: Logically sound, technically deep, evidence-linked.
- **81-100**: Industry-standard rigor, predictive logic, impeccable engineering aesthetics.
